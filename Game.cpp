...
void make_vertexShaders1(GLuint& shader);
void make_fragmentShaders2(GLuint& shader);
...
GLuint object_shaderID;
GLuint object_vertexShader;
GLuint object_fragmentShader;
...
"}\0";
void make_vertexShaders1(GLuint& vertexShader)
{
...
void make_fragmentShaders1(GLuint& fragmentShader)
{
...
GLuint VAO_terrain, VBO_pos_terrain, VBO_uv_terrain, VBO_normal_terrain, EBO_terrain;
...
void InitBuffer()
{
    ...
    glGenBuffers(1, &VBO_pos_terrain);
    glGenBuffers(1, &VBO_uv_terrain);
    glGenBuffers(1, &VBO_normal_terrain);
    glBindVertexArray(VAO_terrain);
    glBindBuffer(GL_ARRAY_BUFFER, VBO_pos_terrain);
    glBufferData(GL_ARRAY_BUFFER, sizeof(TerrainVertices), TerrainVertices, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
    glEnableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, VBO_uv_terrain);
    glBufferData(GL_ARRAY_BUFFER, sizeof(TerrainVTs), TerrainVTs, GL_STATIC_DRAW);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), 0);
...
bool view_on_side = false;
float distance_ball = 0.0f;
float dis = 60.0f;
float rotate_y = 0.0f;
void drawScene()
{
    ...
    if (!view_on_side) {
        ...
        glm::vec3 cameraPos = glm::vec3(75.0f + cameraX, 100.0f + cameraY, 35.0f + cameraZ); //--- 카메라 위치
        glm::vec3 cameraDirection = glm::vec3(0.0f, 10.0f, 42.0f); //--- 카메라 바라보는 방향
        glm::vec3 cameraUp = glm::vec3(0.0f, 1.0f, 0.0f); //--- 카메라 위쪽 방향
        glm::mat4 view = glm::mat4(1.0f);
        glm::mat4 full_camera_rotate = glm::mat4(1.0f); //카메라 공전
        glm::mat4 box_camera_rotate = glm::mat4(1.0f); //배경 회전
    ...
    glBindTexture(GL_TEXTURE_2D, Terrain_load.texture);
    glBindVertexArray(VAO_terrain);
    glm::mat4 Terrain_TF = glm::mat4(1.0f);
    glUniformMatrix4fv(transLoc, 1, GL_FALSE, glm::value_ptr(Terrain_TF));
    glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, (char*)(0 * sizeof(float)));
    ...
    glBindTexture(GL_TEXTURE_2D, CastleGround_load.texture);
    glBindVertexArray(VAO_ground);
    Ground_TF = glm::mat4(1.0f);
    glm::mat4 trans = glm::mat4(1.0f);
    trans = glm::translate(trans, glm::vec3(0.0f, 0.0f, 75.0f));
    Ground_TF= trans;
    glUniformMatrix4fv(transLoc, 1, GL_FALSE, glm::value_ptr(Ground_TF));
    glDrawArrays(GL_TRIANGLES, 0, Ground);
    ...
    Ground_TF = glm::mat4(1.0f);
    trans = glm::mat4(1.0f);
    trans = glm::translate(trans, glm::vec3(58.0f, 0.0f, 0.0f));
    Ground_TF = trans;
    glUniformMatrix4fv(transLoc, 1, GL_FALSE, glm::value_ptr(Ground_TF));
    glDrawArrays(GL_TRIANGLES, 0, Ground);
    */
    ...
    unsigned int transobj = glGetUniformLocation(object_shaderID, "transform");



    glm::mat4 MoveAll = glm::mat4(1.0f);
   MoveAll = glm::translate(MoveAll, glm::vec3(23.5f, 12.f, 23.5f));
    ...
    Rotate =  glm::rotate(Rotate, glm::radians(rotate_y), glm::vec3(1.0f, 0.0f, 0.0f));
    Trigger_TF = MoveAll*Rotate;
    ...
    MoveAll = glm::mat4(1.0f);
    MoveAll = glm::translate(MoveAll, glm::vec3(-22.0f, 12.f, 23.5f));
    glUniform1f(twoTextures, 1);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, Catapult_wood_load.texture);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, Catapult_metal_load.texture);
    glBindVertexArray(VAO_trigger);
    Trigger_TF = glm::mat4(1.0f);
    Trigger_TF = MoveAll;

    glUniformMatrix4fv(transLoc, 1, GL_FALSE, glm::value_ptr(Trigger_TF));
    glDrawArrays(GL_TRIANGLES, 0, Trigger);
    glBindVertexArray(VAO_body);
    Body_TF = glm::mat4(1.0f);
    Body_TF = MoveAll;
    glUniformMatrix4fv(transLoc, 1, GL_FALSE, glm::value_ptr(Body_TF));
    glDrawArrays(GL_TRIANGLES, 0, Body);
    ...
    glm::mat4 TransformationVec = glm::mat4(1.0f);
    glm::mat4 tran = glm::mat4(1.0f);
    glm::mat4 shoot = glm::mat4(1.0f);
    tran = glm::translate(tran, glm::vec3(23.5f, 14.f, 19.5f));
    shoot = glm::translate(shoot, glm::vec3(0.0f, -(distance_ball*distance_ball)/25.0f + distance_ball * dis/25.0f, distance_ball));
    //shoot = glm::translate(shoot, glm::vec3(0.0f,sin(dis-10.0f)*(dis-10.0f), distance_ball));
    TransformationVec = shoot*tran;
    glUniform3f(glGetUniformLocation(object_shaderID, "vColor"), 1.0f, 1.0, 1.0);
    glUniformMatrix4fv(transobj, 1, GL_FALSE, glm::value_ptr(TransformationVec));
    qobj = gluNewQuadric();
...
void Timer(int value) {
    if (go) {
        if (-(distance_ball * distance_ball) / 20.0f + (distance_ball * dis) / 20.0f>-16.0f) {
            distance_ball += 0.2;
...
int main(int argc, char** argv)
{
    ...
    make_vertexShaders1(object_vertexShader);
    make_fragmentShaders1(object_fragmentShader);
    object_shaderID = make_shaderProgram(object_vertexShader, object_fragmentShader);
    InitBuffer();
    Terrain_load.InitTexture("terrain.jpg");
